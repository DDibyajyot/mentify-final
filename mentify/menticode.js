...
d3.csv("mentor.csv").then(function (data) {
var mentor = data;
...
preference1.filter(mentor => mentor.preference1.includes(inputValue));
...
var output = _.sortBy(filterPreference, 'rating').reverse()
...
for (var i = 0; i < filteredPreference.length; i++) {
d3.select("tbody").insert("tr").html(
"<td>" + [i+1] + "</td>" +
"<td>" + (output[i]['name'])+"</a>"+"</td>" +
"<td>" + (output[i]['age'])+"</td>" +
"<td>" + (output[i]['preference1'])+"</td>" +
"<td>" + (output[i]['preference2'])+"</td>" +
